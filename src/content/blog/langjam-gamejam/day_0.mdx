...
In the past, I've been burned by a uniquely cruel combination of overambition and techno-masochism: ideas that work beautifully in my head often find their resting place in implementation hell.

For this project in particular, I am making deliberate effort to make visible, usable progress after every working day.

Behind the scenes, I am on the tail end of a trip to Germany, and have been hacking away on a high-speed train from Berlin to Stuttgart, something my American brain can barely conceive of.
...
pub fn setup_demo_data(grid: &mut GridState) {
    ...
    // show all numbers from 0 to 9 row 5 / 6
    for i in 0..10 {
...
My bitmap font will not cut it for long. I want to render text properly in cells, and allow for text input. I'm considering adding in [Blitz](https://github.com/DioxusLabs/blitz) or something similar to render complicated scenes inside of cells.
...
#### Web target
I _love_ having my artifacts accessible on the web. This gets a little tricky with Bevy, as there is no trivial way to run a WASM application -- especially one that uses a canvas -- on anything other than the main thread. [I have done it once before](https://github.com/gcrois/rust-canvas/), but not without significant complications.
...
I will not entertain baseless accusations that the previous LaTeX was an excuse to add math rendering to my website...
...
(Day 1 is available here: [day_1.mdx]((/blog/langjam-gamejam/day_1)))
